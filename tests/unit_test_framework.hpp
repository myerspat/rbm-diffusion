...
#define ASSERT_NOT_EQUAL(first, second)                                        \
  assert_not_equal(                                                            \
    (first), (second), __LINE__, "ASSERT_NOT_EQUAL(" #first ", " #second ")");
...
using enable_if_equality_comparable = typename std::enable_if<
  std::is_same<bool,
    decltype(std::declval<First>() == std::declval<Second>())>::value and
    std::is_same<bool,
      decltype(std::declval<First>() != std::declval<Second>())>::value and
...
struct is_equality_comparable<First, Second,
  enable_if_equality_comparable<First, Second>> : std::true_type {};
...
template<typename First, typename Second>
void assert_equal(
  First&& first, Second&& second, int line_number, const char* assertion_text)
{
...
int TestSuite::run_tests(int argc, char** argv)
{
  ...
  auto num_failures = std::count_if(tests_.begin(), tests_.end(),
    [](std::pair<std::string, TestCase> test_pair) {
      return not test_pair.second.failure_msg.empty();
    });
  auto num_errors = std::count_if(tests_.begin(), tests_.end(),
    [](std::pair<std::string, TestCase> test_pair) {
